...
import { dashboard, bitable, DashboardState, IConfig, IDashboardTheme } from "@lark-base-open/js-sdk";
import { Button, DatePicker, ConfigProvider, Checkbox, Row, Col, Input, Switch } from '@douyinfe/semi-ui';
// import {} from '@douyinfe/semi-uidist';
import { useState, useEffect, useRef, useCallback } from 'react';
import { getTime } from './utils';
import { updateTheme, useConfig } from '../../hooks';
...
export default function CountDown() {
  ...
  useEffect(() => {
    async function test() {
      ...
      console.log('??? theme', await dashboard.getTheme())
  ...
  return (
    <main style={{ background: 'transparent' }} className={classnames({
      ...
      {
        isConfig && <ConfigPanel themeConfig={themeConfig} t={t} config={config} setConfig={setConfig} availableUnits={availableUnits} />
      }
    </main>
  )
}
    ...
    <div className='config-panel'>
      <div className='form'>
        ...
        <Item label={t("label.color")}>
          <ColorPicker colors={(props.themeConfig?.labelColorTokenList ?? []).map(token => ({ value: token }))} value={config.color} onChange={(v) => {
            setConfig({
              ...config,
              color: v,
            })
